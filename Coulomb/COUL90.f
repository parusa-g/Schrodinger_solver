C----------------------------------------------------------------------
      SUBROUTINE COUL90(X, ETA, LRANGE, FC,GC,FCP,GCP, KFN,IFAIL)
C----------------------------------------------------------------------
C
...
C----------------------------------------------------------------------
      ...
      DOUBLE PRECISION,INTENT(OUT) ::  FC(0:LRANGE), GC(0:LRANGE)
      DOUBLE PRECISION,INTENT(OUT) ::  FCP(0:LRANGE), GCP(0:LRANGE)
C----- ARRAYS INDEXED FROM 0 INSIDE SUBROUTINE: STORAGE FROM MINL
      DOUBLE PRECISION :: ETA_
      DOUBLE PRECISION :: ACCUR,ACCH,SMALL
      DOUBLE PRECISION :: ONE,ZERO,HALF,TWO,TEN2, RT2DPI
      DOUBLE PRECISION :: XINV,PK,CF1,C,D,PK1,ETAK,RK2
      DOUBLE PRECISION :: TK,DCF1,DEN,XLM,XLL
      DOUBLE PRECISION :: EL,XL,RL,SL, F,FCMAXL,FCMINL,GCMINL
      DOUBLE PRECISION :: OMEGA,WRONSK
      DOUBLE PRECISION :: WI, A,B, AR,AI,BR,BI,DR,DI,DP,DQ, ALPHA,BETA
      DOUBLE PRECISION :: E2MM1, FJWKB,GJWKB, P,Q,PACCQ
      DOUBLE PRECISION :: GAMMA,GAMMAI, XLMIN
      INTEGER          :: IEXP, NFP, NPQ, L, MINL,MAXL, LIMIT
      LOGICAL          :: ETANE0, XLTURN
      PARAMETER      ( LIMIT = 20000, SMALL = 1.0D-150, XLMIN=0.d0 )
...
C-----   TEST RANGE OF X, EXIT IF.LE.DSQRT(ACCUR) OR IF NEGATIVE
      ...
      E2MM1  = XLM * XLM + XLM
      XLTURN = X * (X -  TWO * ETA_) .LT. E2MM1
      E2MM1  = E2MM1  +  ETA_ * ETA_
...
C----- BEGIN CF1 LOOP ON PK = K STARTING AT LAMBDA + 1: LENTZ-THOMPSON
   ...
   20    NFP = NINT(PK - XLL - 1)                        ! number of steps
...
C---------------------------------------------------------------------
      IF( XLTURN ) CALL JWKB( X,ETA_,DMAX1(XLM,ZERO),FJWKB,GJWKB,IEXP )
